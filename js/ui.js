/* ============================================================
   WorkFlow — Small UI helpers: icons, toasts, avatars, badges
   ============================================================ */
...
const ICONS = {
  ...
  team: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
  reports: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><rect x="7" y="12" width="3" height="6"/><rect x="12.5" y="8" width="3" height="10"/><rect x="18" y="5" width="3" height="13"/></svg>`,
  capacity: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 10h18"/><path d="M7 14h4M7 17h7"/></svg>`,
  ...
  chevronLeft: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>`,
  ...
  paperclip: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>`,
  ...
  sort: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7h11M3 12h7M3 17h4M17 4v16M17 20l3-3M17 20l-3-3"/></svg>`,
...
function initialsAvatar(userId, size) {
  const u = getUser(userId);
  const cls = size ? "avatar " + size : "avatar";
  return `<div class="${cls}" style="background:${u.color}" data-tooltip="${escapeHtml(u.name)}">${initials(u.name)}</div>`;
}

function projectChip(projectId) {
  const p = getProject(projectId);
  return `<span class="project-chip"><span class="project-dot" style="background:${p.color}"></span>${escapeHtml(p.name)}</span>`;
}
...
/* ---------- Theme ----------
   A per-browser preference, deliberately kept out of Store/the server — everyone on
   the shared team data can have their own light/dark choice. */
const THEME_KEY = "workflow_theme_pref";
